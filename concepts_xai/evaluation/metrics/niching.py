...
def niche_completeness(c_pred, y_true, predictor_model, niches):
    ...
    for task in range(n_tasks):
        ...
        niche[:, niches[:, task] > 0] = c_test[:, niches[:, task] > 0]
...
def niche_purity(c_pred, y_true, predictor_model, niches):
    ...
    for task in range(n_tasks):
        ...
        niche_out[:, niches[:, task] <= 0] = c_test[:, niches[:, task] <= 0]
        ...
        niche_purity = accuracy_score(y_test[:, task], y_pred_niche[:, task])
        # niche impurity: the task accuracy obtained using concepts OUTSIDE the niche w.r.t.
        niche_impurity = accuracy_score(y_test[:, task], y_pred_niche_out[:, task])
