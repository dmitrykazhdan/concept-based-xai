...
class LatentFactorData(object):
    ...
    """
    def __init__(self, dataset_path, task_name, num_factors, sample_shape, c_names, task_fn):
    ...
    def _load_x_c_data(self):
        raise NotImplementedError("Need to implement code for loading input and concept data")
    ...
    def _get_generators(self, train_size=0.85, random_state=42):
        ...
        self.n_c_vals_list     = [len(np.unique(c_data[:, i])) for i in range(c_data.shape[1])]

        # Create dictionary of concept_id --> dictionary: consequtive_id : original_id
        self.cid_new_to_old = []
        for i in range(c_data.shape[1]):
            unique_vals, unique_inds = np.unique(c_data[:, i], return_inverse=True)
            self.cid_new_to_old.append({i : unique_vals[i] for i in range(len(unique_vals))})
            c_data[:, i] = unique_inds

        x_train, x_test, y_train, y_test, c_train, c_test = train_test_split(x_data, y_data, c_data,
                                                                             train_size=train_size,
                                                                             random_state=random_state)

        self.n_train_samples    = c_train.shape[0]
        self.n_test_samples     = c_test.shape[0]
        self.data_gen_train     = tf.data.Dataset.from_tensor_slices((x_train, c_train, y_train))
        self.data_gen_test      = tf.data.Dataset.from_tensor_slices((x_test, c_test, y_test))
