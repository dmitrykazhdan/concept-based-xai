...
class GroupVAEBase(BaseVAE):
    ...
    def __init__(self, latent_dim, encoder_fn, decoder_fn, loss_fn, input_shape, beta=1,  **kwargs):
        ...
        """
        super(GroupVAEBase, self).__init__(latent_dim, encoder_fn, decoder_fn, loss_fn, input_shape, **kwargs)
        self.beta = beta
        self.metric_names.append("regularizer")
        self.metrics_dict["regularizer"] = tf.keras.metrics.Mean(name="regularizer")
    ...
    def _split_sample_pairs(self, x):
        '''
        Note: each point contains two frames stacked along the first dim and integer labels.
        '''
        data_shape    = x.get_shape().as_list()[1:]
        assert data_shape[0] % 2 == 0, "1st dimension of concatenated pairs assumed to be even"
        data_shape[0] = int(data_shape[0] / 2)
        x_1, x_2      = x[:, :data_shape[0], :, :], x[:, data_shape[0]:, :, :]
    ...
    def _compute_losses_weak(self, x_1, x_2, is_training, labels):
        ...
        z_sampled_1 = self.sample_from_latent_distribution(mean_sample_1, log_var_sample_1)
        z_sampled_2 = self.sample_from_latent_distribution(mean_sample_2, log_var_sample_2)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            reconstruction_loss, regularizer, loss, elbo = self._compute_losses_weak(x_1, x_2, is_training, labels)

        gradients = tape.gradient(loss, self.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, self.trainable_variables))
        self.update_metrics([("loss", loss), ("reconstruction_loss", reconstruction_loss), ("elbo", elbo), ("regularizer",regularizer)])
...
class GroupVAEArgmax(GroupVAEBase):
  ...
  def aggregate(self, z_mean, z_logvar, new_mean, new_log_var, labels, kl_per_point):
    return aggregate_argmax(z_mean, z_logvar, new_mean, new_log_var, labels, kl_per_point)
...
class GroupVAELabels(GroupVAEBase):
  ...
  def aggregate(self, z_mean, z_logvar, new_mean, new_log_var, labels, kl_per_point):
    return aggregate_labels(z_mean, z_logvar, new_mean, new_log_var, labels, kl_per_point)
...
def aggregate_argmax(z_mean, z_logvar, new_mean, new_log_var, labels, kl_per_point):
  ...
  Args:
    ...
    z_logvar: Logvar of the encoder distribution for the original image.
    new_mean: Average mean of the encoder distribution of the pair of images.
    new_log_var: Average logvar of the encoder distribution of the pair of images.
    labels: One-hot-encoding with the position of the dimension that should not be shared.
